...
- 默认以 Windows 托盘程序运行，不显示黑色控制台窗口。
- 右键托盘图标可切换自动分流、强制 Tyty、强制 GlobalProtect、本地直连。
- 双击托盘图标可打开状态窗口，查看当前模式、代理状态、VPN 网卡状态和最近日志。
...
- 自动分流：按 `config.yaml` 的域名规则自动选择 Tyty 或 GlobalProtect。
- 强制 Tyty：暂停域名自动切换，非直连请求强制走 Tyty。
- 强制 GlobalProtect：暂停域名自动切换，非直连请求强制走 GlobalProtect。
- 本地直连：恢复系统代理，让浏览器回到本地网络。
- 恢复系统代理：手动恢复启动前的系统代理配置。
- 打开状态窗口：显示当前状态和最近日志。
- 退出：关闭程序并恢复系统代理。
