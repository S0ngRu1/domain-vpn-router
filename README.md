...
- 默认以 Windows 托盘程序运行。
- 右键或双击托盘图标会显示轻量小浮层菜单，不弹出常驻大窗口。
- 小浮层可查看当前模式、物理网卡 IP、Clash Verge 网卡状态、GlobalProtect 网卡状态和上次刷新时间。
- 支持在小浮层里手动刷新网卡状态；程序不会定时轮询 PowerShell 或反复重绘界面。
...
- 物理网卡 IP 在启动或手动刷新/保存配置时检测并缓存，请求转发路径不会每次枚举网卡。
- 日志默认只保存在内存里，且不记录每个请求，避免高并发时被日志拖慢。
...
为降低系统负载，自动模式下不会持续查询网卡状态。托盘小浮层显示的是最近一次缓存结果；需要重新检测 Clash Verge、GlobalProtect 或物理网卡 IP 时，点击小浮层里的 `刷新网卡状态`。
...
- 右键托盘图标：打开小浮层菜单。
- 双击托盘图标：打开小浮层菜单。
- `刷新网卡状态`：手动检测 Clash Verge、GlobalProtect 和物理网卡 IP。
- `自动分流`：按 `config.yaml` 的规则分流。
- `强制 Clash`：非直连规则尽量走 Clash Verge。
- `强制 GlobalProtect`：非直连规则尽量走 GP。
- `本地直连`：恢复系统代理，流量走本地直连。
- `恢复系统代理`：手动恢复启动前的 Windows 系统代理设置。
- `退出`：退出程序并恢复系统代理。
...
日志默认保存在内存中，不写入磁盘文件。右键或双击托盘图标打开小浮层后，可以看到当前状态；如果需要查看最近日志，可在仍保留的窗口日志页中查看最近一页。

为了保持轻量和高并发性能，程序不会记录每个请求的访问日志。目前主要记录：
...
- 如果更换了网络、切换了 Wi-Fi 或重新连接 VPN，请点击托盘小浮层里的 `刷新网卡状态`，让程序更新缓存的网卡信息。
